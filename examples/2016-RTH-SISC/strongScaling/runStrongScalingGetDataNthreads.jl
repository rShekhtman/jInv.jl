...
include("getDCResistivitySourcesAMG.jl")
...
function main()
	...
	elseif solver==2
		...
		Ainv.nthreads=nthreads
		Ainv.out     =-1
		Ainvt         = getBlockIterativeSolver(KrylovMethods.blockCG,out=-1,sym=1,PC=:jac);
		Ainvt.maxIter = 1000
		Ainvt.tol     = 1e-8
		Ainvt.nthreads=nthreads
		Ainvt.out     =-1
	...
	elseif solver 		== 4
		...
		strongConnParam = 0.25;
		MG 				= getMGparam(levels,numCores,maxIter,relativeTol,relaxType,relaxParam,relaxPre,relaxPost,cycleType,coarseSolveType,strongConnParam);
