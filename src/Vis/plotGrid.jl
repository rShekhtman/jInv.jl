...
"""
function plotGrid(y,M;spacing=[1,1,1],color="b")
    ...
    if M.dim==2
        ...
        p1 = plot(y1[:,J2],y2[:,J2],color=color);
        p2 = plot(y1[J1,:]',y2[J1,:]',color=color);
    elseif M.dim==3
        pt1 = p3(yi(1),yi(2),yi(3),nn,[1 2 3],spacing,color)
        pt2 = p3(yi(1),yi(2),yi(3),nn,[2,1,3],spacing,color);
        pt3 = p3(yi(1),yi(2),yi(3),nn,[3,1,2],spacing,color);
	else
...
function p3(y1,y2,y3,m,order,s,color)
    ...
    K  = vec(K2)+vec(K3);
    for k=1:length(K)
        plot3D(y1[:,K[k]],y2[:,K[k]], y3[:,K[k]],color=color)
    end
end
